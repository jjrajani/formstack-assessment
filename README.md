...
___
## Getting Started
To get you started you can simply clone the `formstack-assessment` repository and install the dependencies:

#### Prerequisites

* You need git to clone the `formstack-assessment` repository. You can get git from [here][git].
* You also need Node.js tools to initialize and test `formstack-assessment`. You can get that from [here][node].

#### Clone `formstack-assessment`

Clone the `formstack-assessment` repository using git:
...
If you just want to start a new project without the `formstack-assessment` commit history then you can do:

```
git clone --depth=1 https://github.com/jjrajani/formstack-assessment.git <your-project-name>
```
...
`formstack-assessment` changes this location through the `.bowerrc` file. Putting it in the `app` folder
makes it easier to serve the files by a web server.*
___
## Run the Application

AngularJS has preconfigured the project with a simple development web server. The simplest way to start
this server is:
...
There are two kinds of tests in the `formstack-assessment` application: Unit tests and end-to-end tests.

### Running Unit Tests

The `formstack-assessment` app comes preconfigured with unit tests. These are written in [Jasmine][jasmine],
which we run with the [Karma][karma] test runner. Angular provides a Karma configuration file to run them.
...
The `formstack-assessment` app comes with end-to-end tests, again written in [Jasmine][jasmine]. These tests are run with the [Protractor][protractor] End-to-End test runner. It uses native events and has special features for Angular applications.
...
The `formstack-assessment` project comes preconfigured with a local development web server. It is a Node.js tool called [http-server][http-server]. You can start this web server with `npm start`, but you may choose to install the tool globally:
...
[Travis CI][travis] is a continuous integration service, which can monitor GitHub for new commits to your repository and execute scripts such as building the app or running tests. The `formstack-assessment` project contains a Travis configuration file, `.travis.yml`, which will cause Travis to run your tests when you push to GitHub.
